...
for i in range(n_agent):
    ...
    Pi.append(PPOPolicyNetwork(num_features=env.input_size+more_obs_size+n_actions, num_actions=n_actions, layer_size=64, epsilon=0.1,
                               learning_rate=lr_actor))
    gV.append(ValueNetwork(num_features=env.input_size, hidden_size=256, learning_rate=0.001))
    V.append(ValueNetwork(num_features=env.input_size+more_obs_size+n_actions, hidden_size=256, learning_rate=0.001))

memory_ep_rewards = [deque() for _ in range(n_agent)]
average_jpi = np.zeros(n_agent)
laverage_jpi = np.zeros((n_agent, n_agent))
...
while i_episode < n_episode:
    ...
    while steps < max_steps and not done:
        ...
        #update beliefs
        for i in range(n_agent):
            laverage_jpi[i][i] = average_jpi[i] #know my own value
            (neigh, index) = neighbors[0][i], neighbors[1][i]
            neigh = neigh[0:len(index)]
            laverage_jpi[i][neigh] = average_jpi[neigh]
        ...
        if steps % T == 0:
            ...
            all_sorted_index=[]
            for i in range(n_agent):
                sorted_index = laverage_jpi[i].argsort()
            ...
            for i in range(n_agent):
                if greedy[i]:
                    ...
                    advApproOthers = np.zeros((n_agent, T))
                    advApproOthers[i] = all_ep_advantages[i]
                    (neigh, index) = neighbors[0][i], neighbors[1][i]
                    neigh = neigh[0:len(index)]
                    advApproOthers[neigh, :] = all_ep_advantages[neigh]

                    ep_advantages = ((average_jpi+0.0000000001)**-alpha_fairness) @ advApproOthers
                    ep_advantages = (ep_advantages - np.mean(ep_advantages)) / (np.std(ep_advantages) + 0.0000000001)
