...
class TrafficSignal:
    ...
    # Umer
    def get_waiting_time_of_lane2(self):
        wait_time_per_lane = []
        for lane in self.lanes:
            ...
            if veh_num == 0:
                ...
                wait_time_per_lane.append(wait_time/veh_num)"""
        return wait_time_per_lane

    # def get_waiting_time_of_lane2(self):
    #     return [traci.lane.getWaitingTime(lane) for lane in self.lanes]
     
    def get_waiting_time_of_lane(self):
        vehicle_size_min_gap = 7.5  # 5(vehSize) + 2.5(minGap)
        return [min(1, traci.lane.getWaitingTime(lane) / (traci.lane.getLength(lane) / vehicle_size_min_gap)) for lane in self.lanes]
