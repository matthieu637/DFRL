...
class PPOPolicyNetwork():
    def __init__(self, num_features, layer_size, num_actions, num_agent, epsilon=.1,
        ...
        with self.tf_graph.as_default():
            ...
            self.sorted_omega = tf.placeholder(shape=[num_agent], dtype=tf.float32)
            self.advantages_aggregated = tf.reduce_sum(tf.multiply(self.advantages, self.sorted_omega), axis=1)
            ...
            self.loss -= 0.03 * self.new_responsible_outputs * tf.log(self.new_responsible_outputs + 1e-10)
    ...
    def update(self, states, chosen_actions, ep_advantages, sorted_omega):
        ...
        self.gradients, loss = self.session.run([self.get_grad, self.output], feed_dict={
            ...
            self.sorted_omega: sorted_omega
        })
