...
class Env():
    ...
    def __init__(self, normalize, T=50, one_hot_encoding=True):
        #shared parameters
        ...
        self.n_neighbors = 4
        if one_hot_encoding:
            self.input_size = 80#25*3+3+2
        else:
            self.input_size = 30#25+3+2
        self.GAMMA = 0.98
        self.n_resource = 8
        self.one_hot_encoding = one_hot_encoding

        self.fileresults = open('learning.data', "w")
        self.fileresults2 = open('learningru.data', "w")
    ...
    def step(self, action):
        ...
        for j in range(self.n_resource):
            for i in range(self.n_agent):
                if (self.ant[i][0] == self.resource[j][0]) & (self.ant[i][1] == self.resource[j][1]):
                    ...
                    #bug found:
                    #if self.possession[i][self.resource_type[j]] < self.requirement[i][self.resource_type[j]]:
                    re[i] += 0.01
        ...
        self.rinfo += re2
        return self._get_obs(), re, self.number >= 800

    def end_episode(self):
        self.fileresults.write(','.join(self.rinfo.flatten().astype('str')) + '\n')
        self.fileresults.flush()
        self.fileresults2.write(','.join(self.rinfo2.flatten().astype('str')) + '\n')
        self.fileresults2.flush()
